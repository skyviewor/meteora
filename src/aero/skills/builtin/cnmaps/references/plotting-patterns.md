...
For EPS/PS export compatibility with complex boundaries:
...
contours = ax.contourf(lons, lats, speed, levels=np.linspace(speed.min(), speed.max(), 50),
                       cmap=plt.cm.RdYlBu_r, transform=ccrs.PlateCarree(), zorder=1)
...
lons, lats, dem = load_dem()
hillshade = LightSource(azdeg=315, altdeg=45).shade(dem, cmap=plt.cm.Greys, vert_exag=0.8, blend_mode="overlay")
china = get_adm_maps(country="中国", level="国", record="first", only_polygon=True)

fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111, projection=ccrs.PlateCarree())
image = ax.imshow(hillshade, extent=[lons.min(), lons.max(), lats.min(), lats.max()],
...
fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111, projection=ccrs.PlateCarree())
stream = ax.streamplot(lons[0, :], lats[:, 0], u, v, transform=ccrs.PlateCarree(), density=1.1, color="#1f77b4")
...
lons, lats, u, v = load_wind()
speed = (u**2 + v**2) ** 0.5
x, y, z = lons.flatten(), lats.flatten(), speed.flatten()
china = get_adm_maps(country="中国", level="国", record="first", only_polygon=True)

fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111, projection=ccrs.PlateCarree())
scatter = ax.scatter(x, y, s=z * 10, c=z, cmap=plt.cm.RdYlBu_r, transform=ccrs.PlateCarree())
...
fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111, projection=ccrs.PlateCarree())
contours = ax.contour(lons, lats, dem, levels=np.linspace(-2500, dem.max(), 20),
...
for city_name, label in capitals:
    ...
    ax.scatter(record.longitude, record.latitude, s=10, color="crimson", transform=ccrs.PlateCarree())
    ax.text(record.longitude, record.latitude + 0.3, label, ha="center", va="bottom", transform=ccrs.PlateCarree())
