...
- Mixed levels are not supported in one call: if the user wants `日本` and `四川省`, query them separately.
...
- `longitude` and `latitude` are already included on `MapRecord` results and are the default coordinates for map labels or marker placement.
...
Use for a single `MapPolygon` or a single geometry from a record.

### `draw_maps(maps, ax=None, **kwargs)`

Use for lists of records, lists of polygons, `GeoDataFrame`, a single `MapRecord`, or a single `MapPolygon`.
...
- `draw_map(..., autoscale=True)` auto-scales by default; user code may still call `ax.set_extent(...)` explicitly for tighter framing.
...
Use after `ax.imshow`, especially for hillshade or RGB image layers.
...
Typical order for contour labels: create contours → `clip_contours_by_map` → create labels with `ax.clabel` → `clip_clabels_by_map`.
...
- If the user is exporting clipped `contourf` results to EPS/PS and the output becomes unreadable, prefer querying the clipping boundary with `simplify=True` before calling `clip_contours_by_map(...)`.
...
Use when the user wants to export administrative boundaries directly from the terminal.
...
Multi-name filters are passed as repeated values after one flag.
...
- If the user wants exact regions from different administrative levels, split the task into multiple queries.
- If the user wants to label administrative regions: query records and use `longitude` and `latitude`.
- If the user wants actual clipped scientific plots: combine sample loaders, `cartopy`, and `clip_*` helpers.
- If the user wants clipped scientific plots specifically for EPS/PS export, consider `simplify=True` on the clipping boundary.
- If the user wants a raster mask array: use `MapPolygon.make_mask_array(...)` or `MapPolygon.maskout(...)`.
- If the user wants exported vector output: query `only_polygon=True` and use `map_polygon.to_file(...)`.
- If the user wants to use a custom GeoJSON or Shapefile for masking, validate it against the cnmaps boundary spec first, then read with `read_boundary_file(...)`.
