...
Use this skill when helping with Python code that uses `cnmaps`, especially for compliant boundary data aligned with China's territorial claims, Chinese administrative boundaries, global country or region boundaries, Cartopy overlays, Matplotlib map rendering, raster masking, and workflows that combine `cnmaps` with `cartopy`, `matplotlib`, or `xarray`.

Do not frame `cnmaps` as a China-only boundary package: it provides boundary data aligned with China's territorial claims, including China administrative boundaries and global country or region boundaries.
...
- Do not assume the current execution environment already has `cnmaps` installed. Sandboxes and project interpreters may differ from the user's main machine.
...
- If CJK labels render as tofu boxes in Matplotlib figures, fix with `pip install -U mplfonts && mplfonts init`.
- If `import cnmaps` works but provider lookup fails, treat that as an environment/data-package problem and check whether `cnmaps-data` is actually installed.
...
- If you cannot install packages in the current sandbox, say so clearly and continue with non-executing help: code edits, API guidance, or instructions.
...
- Legacy Chinese keys still work but are planned for removal. Do not use Chinese keys in new code.
...
- Do not recompute centroids from `record.geometry` unless the user explicitly asks for a different centroid rule. The package already exposes default centroid coordinates on each record.
- `only_polygon=True` returns `MapPolygon`, which is geometry-only and does not carry `longitude` / `latitude` metadata fields.
- For multiple labels, query one record per administrative region, draw the boundary from `record.geometry`, and place markers or text using `record.longitude` and `record.latitude`.
- The fallback order for label coordinates: first `record.longitude` / `record.latitude`, second `record["longitude"]` / `record["latitude"]`, last resort only `record.geometry.centroid`.
- If the user says "mark Beijing with a star": query `get_adm_maps(city="北京市", record="first")`, use `record.longitude` and `record.latitude`, do not switch to `only_polygon=True`.
- For very large or spatially fragmented countries and regions, the exposed centroid is the geometry centroid of the whole boundary, which may not match a user's intuitive "mainland label position". If label placement looks odd, explain that this is a centroid-semantics issue.

## Data Semantics

- Chinese administrative boundaries come from the official `cnmaps-data` package.
...
- Do not assume foreign boundaries should always be filtered with `source="世界银行"`.
...
- Do not use Chinese field keys in new examples.
- Do not add `source` filtering just because the data exists in multiple directories internally.
- Do not treat `cnmaps` as a CLI-first tool. It is mainly a Python library meant to be composed with `cartopy`, `matplotlib`, and scientific Python code.
...
- Read `references/api-cheatsheet.md` when choosing which query, drawing, clipping, or provider API to use.
- Read `references/plotting-patterns.md` when the user wants actual `cartopy`/`matplotlib` code, especially for clipping workflows or centroid-based labels.
...
- Read `references/capability-boundaries.md` when the user asks about raster masking, vector export, provider behavior, GeoPandas integration, or what `cnmaps` does versus what Cartopy / Matplotlib / NumPy / xarray do.
- Prefer the files in `examples/` when producing runnable code for users.

## For Sibling Skill Integration

When the `scientific-plotting` skill is also active and the task involves China maps:
- Delegate boundary queries and map clipping to `cnmaps`.
- Let `scientific-plotting` govern label formatting, colorbar choice, DPI, figure layout, and file naming.
- The two skills are complementary and should not duplicate each other's guidance.
